...
async function fetchCustomerData() {
  try {
    ...
    // .json() 메서드
    // : json 데이터를 parse하여 JS 객체로 변환
    let jsonData = await data.json();
    console.log(jsonData);
    

  } catch(error) {
    // try 블록에서 에러 발생시 해당 에러를 매개변수로 받아 처리

    // cf) cer: console.error()
    //    - 콘솔 출략문이 빨간색으로 예외처럼 출력
...
// 1) ok: HTTP 상태코드가 200~299 범위에 있을 때 true를 반환
// >> 요청이 성공적으로 완료되었음을 반환

// 2) status: 응답의 HTTP 상태코드를 반환 (예. 200, 404, 500 등)

// 3) statusText: 응답의 HTTP 상태코드에 대한 텍스트 메시지 반환 (예. "OK", "NOT FOUND", "SERVER ERROR" 등)
