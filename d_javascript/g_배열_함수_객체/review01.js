...
let nextId = 1; // 고유 ID 값을 위한 전역 변수

// 1) 할 일 추가 함수
function addTodo(content) {// 함수 호출 시 인자로 할 일의 내용 전
  // 새로운 할 일 생성
  const newTodo = {
    id: nextId,
    content: content,
  ...
  nextId++; // 다음 할 일 추가 시 아용할 고유 ID 값 증가
  todos.push(newTodo);
...
function toggleTodo(id) {
  // 전체 할 일 목록 중 id값과 일치하는 할 일만 완료상태 전환
  todos = todos.map(todo => {
    ...
    if (todo.id === id) {
      ...
      // cf) 속성키: 새로운 속성값; >> 객체 내부에서 속성값이 재할당
...
function deleteTodo(id) {
  // 1, 2, 3, 4, 5 중 4를 제거
  // 4와 일치하지 않는 1, 2, 3, 5 만을 새로운 배열로 저장
  // >> 4가 삭제된 효과
