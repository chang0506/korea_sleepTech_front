...
// >> 경로값: /
//    ? korea_sleepTech_frot 프로젝트의 전 파일에서 접근 가능
...
//? 쿠키 문자열을 분리
// : 웹 문서에 저장된 쿠키로부터 문자열을 ; (세미콜론)을 기준으로 분리해서
//  , 개별 쿠키를 배열로 반환
...
function getCookieValue(cookieName) {
  ...
  for (let i = 0; i < length; i++) {
    const cookie = cookies[i]; // 배열 순회 + 각 쿠키를 변수에 저장
...
function deleteCookie(cookieName) {
  document.cookie = cookieName + "=; expires=Thu 20 February 2025 00:00:00 GMT; path=/;"
}
