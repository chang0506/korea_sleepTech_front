...
// cf) 화살표 함수 적용
const newArrowNums = numbers.map((value) => value * value); // 중괄호(구현부) 생략 시 return 키워드도 생략
...
// >> 'filter', 'map', 'forEach'
let a = numbersArray.filter(num => num % 2 === 0)
let b = a.map(num => num * num)
b.forEach(num => console.log(num));

numbersArray.filter(num => num % 2 === 0).map(num => num * num).forEach(num => console.log(num));