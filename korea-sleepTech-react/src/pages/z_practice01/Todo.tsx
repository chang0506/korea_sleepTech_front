      ...
      <div css={s.container}>
        ...
        <ul css={s.todoList}>
          {filteredTodos.map((todo) => (
            <li css={s.todoItem} key={todo.id}>
              <span css={s.todoText(todo.completed)} onClick={() => handleToggleTodo(todo.id)}>{todo.text}</span>
              <button css={s.deleteButton} onClick={() => handleDeleteTodo(todo.id)}>삭제</button>
            </li>
          ))}
        </ul>
      </div>
