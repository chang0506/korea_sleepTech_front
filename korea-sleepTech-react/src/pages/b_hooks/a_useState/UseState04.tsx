...
const initaialFormData: IFormData = {
  id: "",
  password: "",
  email: "",
};
...
function UseState04() {
  //! 폼 데이터 상태 관리
  const [formData, setFormData] = useState<IFormData>(initaialFormData);

  const { id, password, email } = formData;

  //! 폼 입력 오류 메시지 상태 관리
  const [errros, setErrors] = useState<IFormData>(initaialFormData);
    ...
    >
      ...
      <form onSubmit={onSignUpSubmit}>
        <div>
          ...
          {errros.id && <p style={{ color: "red" }}>{errros.id}</p>}
        </div>
        <div>
          <label>
            비밀번호
          ...
          {errros.password && <p style={{ color: "red" }}>{errros.password}</p>}
        </div>
        <div>
          <label>
            이메일
          ...
          {errros.email && <p style={{ color: "red" }}>{errros.email}</p>}
        </div>
