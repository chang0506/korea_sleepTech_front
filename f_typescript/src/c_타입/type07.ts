...
function select(name?: string, nickname: string = '서냥화') {
  if (name) {
...
select(); // 서냥화 is guest
select('전창현'); // 서냥화 is 전창현
select('학생'); // 서냥화 is 학생 ******
// >> 선택적 매개변수와 기본 매개변수를 혼합 사용 시
// : 선택적 매개변수는 반드시!! 필수 매개변수(인자로 전달하는 값) 뒤에 작성!!
// - 기본 매개변수는 필수, 선택적 매개변수 양쪽에 어디든 작성 가능
// - 기본 매개변수가 앞서는 경우 생략하려면 반드시 undefined 값을 전달
select(undefined, '학생'); // 학생 is guest
select('문창배', '미어캣');

//# +) Rest(나머지) 매개변수
// : 함수에 전달하는 여러개의 매개 변수를 그룹화하여 배열로 전달하는 기능
...
console.log(sum(1, 2, 10, 20, 30, 40));
